...
void Client::add_product(Product* product){
    ...
    product->modifyQuantity(product->getQuantity());
}
...
ostream& operator<<(ostream& stream,  Client client){
    stream << "Client: " <<client.get_firstname() << " " << client.get_name() << " has " << client.get_products().size() << " product(s)." << endl;
    stream << "------------------------------------------------------------------------------------------" << endl;
    for (unsigned j = 0; j < client.get_products().size(); j++)
    {
        stream << *client.get_products()[j] << endl;   
    }
    return stream;
}
